...
export class WSPacket {
    ...
    static new_from_string(seq: number, cmd: number, msg: string): WSPacket {
        console.log('new ws packet from string:', { seq, cmd, msg });
        const enc = new TextEncoder();
        const content = enc.encode(msg);
        const header = new WSPacketHeader(seq, cmd, content.length);
        return new WSPacket(header, content);
    }

    static new_from_buffer(buf: Uint8Array): WSPacket {
