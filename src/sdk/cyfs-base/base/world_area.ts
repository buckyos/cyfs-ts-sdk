...
export function get_area_info(country_id: string, city_id: string, locale?: string): { country_name: string, state_name: string, city_name: string } | null {
    const nameKey = (!locale || locale === 'zh') ? 'cname' : 'name';

    for (let i = 0; i < countries.length; i++) {
        let country = countries[i];
        if (country["id"] === country_id) {
            let country_name = country[nameKey];
            let state_name = '--';
            let city_name = '--';
            let find = false;
            ...
            for (let j = 0; j < country["states"].length; j++) {
                ...
                for (let x = 0; x < state["cities"].length; x++) {
                    let city = state["cities"][x];
                    if (city["id"] === city_id) {
                        city_name = city[nameKey];
                        state_name = state[nameKey];
                        find = true;
                        break;
                    }
                }
