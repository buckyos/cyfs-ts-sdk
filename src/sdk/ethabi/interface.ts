...
import { checkResultErrors, EthAbiResult } from "./coders/abstract-coder";
...
const BuiltinErrors: Record<string, { signature: string, inputs: string[], name: string, reason?: boolean }> = {
...
export class Interface {
    ...
    constructor(fragments: string | ReadonlyArray<Fragment | JsonFragment | string>) {
        ...
        this.fragments.forEach((fragment) => {
            let bucket: { [ name: string ]: Fragment } | null = null;
            switch (fragment.type) {
                case "constructor":
                    ...
                    defineReadOnly(this, "deploy", fragment as ConstructorFragment);
                    return;
                case "function":
                    // checkNames(fragment, "input", fragment.inputs);
    ...
    encodeFilterTopics(eventFragment: EventFragment | string, values: ReadonlyArray<any>): (string | string[]|null)[] {
        ...
        const encodeTopic = (param: ParamType, value: any): string => {
            ...
            if (param.type === "address") { 
                return this._abiCoder.encode( [ "address" ], [ value ]); 
            }
            return hexZeroPad(hexlify(value), 32);
        };

        values.forEach((value, index) => {
            let param = (eventFragment as EventFragment).inputs[index];
    ...
    encodeEventLog(eventFragment: EventFragment | string, values: ReadonlyArray<any>): { data: string, topics: string[] } {
        if (typeof(eventFragment) === "string") {
            eventFragment = this.getEvent(eventFragment);
        }
    ...
    decodeEventLog(eventFragment: EventFragment | string, data: BytesLike, topics?: ReadonlyArray<string>): EthAbiResult {
        ...
        let indexed: ParamType[] = [];
        let nonIndexed: ParamType[] = [];
        let dynamic: boolean[] = [];
    ...
    parseTransaction(tx: { data: string, value?: BigNumberish }): TransactionDescription | null {
