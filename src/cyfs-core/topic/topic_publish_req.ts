import {
    SubDescType,
    DescTypeInfo, DescContent, DescContentDecoder,
    BodyContent, BodyContentDecoder,
    NamedObjectId, NamedObjectIdDecoder,
    NamedObjectDesc, NamedObjectDescDecoder,
    NamedObject, NamedObjectBuilder, NamedObjectDecoder,
    named_id_gen_default,
    named_id_from_base_58,
    named_id_try_from_object_id,
} from "../../cyfs-base/objects/object"

import { BuckyResult, Ok} from "../../cyfs-base/base/results";
import { BuckyNumber, BuckyNumberDecoder } from "../../cyfs-base/base/bucky_number";
import { ObjectId, ObjectIdDecoder } from "../../cyfs-base/objects/object_id";
import { DeviceId } from "../../cyfs-base/objects/device";

import { CoreObjectType } from "../core_obj_type";
import { TopicId, TopicIdDecoder } from "./topic";

export class TopicPublishReqDescTypeInfo extends DescTypeInfo{
    obj_type() : number{
        return CoreObjectType.TopicPublishReq;
    }

    sub_desc_type(): SubDescType{
        return {
            owner_type: "option",
            area_type: "disable",
            author_type: "disable",
            key_type: "disable"
        }
    }
}

const TOPICPUBLISHREQ_DESC_TYPE_INFO = new TopicPublishReqDescTypeInfo();

export class TopicPublishReqDescContent extends DescContent {
    private readonly m_topic_id: TopicId;
    private readonly m_topic_owner_id: ObjectId;
    private readonly m_member_id: ObjectId;
    private readonly m_seq: bigint;   // u64
    private readonly m_count: number; // u32

    constructor(topic_id: TopicId, topic_owner_id: ObjectId, member_id: ObjectId, seq: bigint, count: number){
        super();
        this.m_topic_id = topic_id;
        this.m_topic_owner_id = topic_owner_id;
        this.m_member_id = member_id;
        this.m_seq = seq;
        this.m_count = count;
    }

    topic_id(): TopicId {
        return this.m_topic_id;
    }

    topic_owner_id(): ObjectId{
        return this.m_topic_owner_id;
    }

    member_id(): ObjectId{
        return this.m_member_id;
    }

    seq(): bigint{
        return this.m_seq;
    }

    count(): number{
        return this.m_count;
    }

    type_info(): DescTypeInfo{
        return TOPICPUBLISHREQ_DESC_TYPE_INFO;
    }

    raw_measure(): BuckyResult<number>{
        let size = 0;
        {
            const r = this.m_topic_id.raw_measure();
            if(r.err){
                return r;
            }
            size += r.unwrap();
        }

        {
            const r = this.m_topic_owner_id.raw_measure();
            if(r.err){
                return r;
            }
            size += r.unwrap();
        }

        {
            const r = this.m_member_id.raw_measure();
            if(r.err){
                return r;
            }
            size += r.unwrap();
        }

        {
            const r =new BuckyNumber('u64',this.m_seq).raw_measure();
            if(r.err){
                return r;
            }
            size += r.unwrap();
        }

        {
            const r =new BuckyNumber('u32',this.m_count).raw_measure();
            if(r.err){
                return r;
            }
            size += r.unwrap();
        }

        return Ok(size);
    }

    raw_encode(buf: Uint8Array): BuckyResult<Uint8Array>{
        {
            const r = this.m_topic_id.raw_encode(buf);
            if(r.err){
                return r;
            }
            buf = r.unwrap();
        }

        {
            const r = this.m_topic_owner_id.raw_encode(buf);
            if(r.err){
                return r;
            }
            buf = r.unwrap();
        }

        {
            const r = this.m_member_id.raw_encode(buf);
            if(r.err){
                return r;
            }
            buf = r.unwrap();
        }

        {
            const r =new BuckyNumber('u64',this.m_seq).raw_encode(buf);
            if(r.err){
                return r;
            }
            buf = r.unwrap();
        }

        {
            const r =new BuckyNumber('u32',this.m_count).raw_encode(buf);
            if(r.err){
                return r;
            }
            buf = r.unwrap();
        }

        return Ok(buf);
    }
}

export class TopicPublishReqDescContentDecoder extends DescContentDecoder<TopicPublishReqDescContent>{
    type_info(): DescTypeInfo{
        return TOPICPUBLISHREQ_DESC_TYPE_INFO;
    }

    raw_decode(buf: Uint8Array): BuckyResult<[TopicPublishReqDescContent, Uint8Array]>{
        let topic_id;
        {
            const r = new TopicIdDecoder().raw_decode(buf);
            if(r.err){
                return r;
            }
            [topic_id, buf] = r.unwrap();
        }

        let topic_owner_id;
        {
            const r = new ObjectIdDecoder().raw_decode(buf);
            if(r.err){
                return r;
            }
            [topic_owner_id, buf] = r.unwrap();
        }

        let member_id;
        {
            const r = new ObjectIdDecoder().raw_decode(buf);
            if(r.err){
                return r;
            }
            [member_id, buf] = r.unwrap();
        }

        let seq;
        {
            const r = new BuckyNumberDecoder('u64').raw_decode(buf);
            if(r.err){
                return r;
            }
            [seq, buf] = r.unwrap();
        }

        let count;
        {
            const r = new BuckyNumberDecoder("u32").raw_decode(buf);
            if(r.err){
                return r;
            }
            [count, buf] = r.unwrap();
        }

        const self = new TopicPublishReqDescContent(topic_id, topic_owner_id, member_id, seq.toBigInt(), count.toNumber());
        const ret:[TopicPublishReqDescContent, Uint8Array] = [self, buf];
        return Ok(ret);
    }
}

export class TopicPublishReqBodyContent extends BodyContent{
    constructor(){
        super();
    }

    raw_measure(): BuckyResult<number>{
        return Ok(0);
    }

    raw_encode(buf: Uint8Array): BuckyResult<Uint8Array>{
        return Ok(buf);
    }
}

export class TopicPublishReqBodyContentDecoder extends BodyContentDecoder<TopicPublishReqBodyContent>{
    raw_decode(buf: Uint8Array): BuckyResult<[TopicPublishReqBodyContent, Uint8Array]>{

        const zone_body_content = new TopicPublishReqBodyContent();

        const ret:[TopicPublishReqBodyContent, Uint8Array] = [zone_body_content, buf];

        return Ok(ret);
    }
}

export class TopicPublishReqDesc extends NamedObjectDesc<TopicPublishReqDescContent>{
    // ignore
}

export  class TopicPublishReqDescDecoder extends NamedObjectDescDecoder<TopicPublishReqDescContent>{
    // ignore
}

export class TopicPublishReqBuilder extends NamedObjectBuilder<TopicPublishReqDescContent, TopicPublishReqBodyContent>{
    // ignore
}

export class TopicPublishReqId extends NamedObjectId<TopicPublishReqDescContent, TopicPublishReqBodyContent>{
    constructor(id: ObjectId){
        super(CoreObjectType.TopicPublishReq, id);
    }

    static default(): DeviceId{
        return named_id_gen_default(CoreObjectType.TopicPublishReq);
    }

    static from_base_58(s: string): BuckyResult<DeviceId> {
        return named_id_from_base_58(CoreObjectType.TopicPublishReq, s);
    }

    static try_from_object_id(id: ObjectId): BuckyResult<DeviceId>{
        return named_id_try_from_object_id(CoreObjectType.TopicPublishReq, id);
    }
}

export class TopicPublishReqIdDecoder extends NamedObjectIdDecoder<TopicPublishReqDescContent, TopicPublishReqBodyContent>{
    constructor(){
        super(CoreObjectType.TopicPublishReq);
    }
}


export class TopicPublishReq extends NamedObject<TopicPublishReqDescContent, TopicPublishReqBodyContent>{
    static create(owner_id: ObjectId, topic_id: TopicId, topic_owner_id: ObjectId, member_id: ObjectId, seq: bigint, count: number):TopicPublishReq{
        const desc_content = new TopicPublishReqDescContent(topic_id, topic_owner_id, member_id, seq, count);
        const body_content = new TopicPublishReqBodyContent();
        const self = new TopicPublishReqBuilder(desc_content, body_content).owner(owner_id).build();
        return new TopicPublishReq(self.desc(), self.body(), self.signs(), self.nonce())
    }

    owner_id(): ObjectId{
        return this.desc().owner()!.unwrap();
    }

    topic_publish_id() :  TopicPublishReqId{
        return new TopicPublishReqId(this.desc().calculate_id());
    }

    topic_id(): TopicId {
        return this.desc().content().topic_id();
    }

    topic_owner_id(): ObjectId{
        return this.desc().content().topic_owner_id();
    }

    member_id(): ObjectId{
        return this.desc().content().member_id();
    }

    seq(): bigint{
        return this.desc().content().seq();
    }

    count(): number{
        return this.desc().content().count();
    }
}

export class TopicPublishReqDecoder extends NamedObjectDecoder<TopicPublishReqDescContent, TopicPublishReqBodyContent, TopicPublishReq>{
    constructor(){
        super(new TopicPublishReqDescContentDecoder(), new TopicPublishReqBodyContentDecoder(), TopicPublishReq);
    }

    raw_decode(buf: Uint8Array): BuckyResult<[TopicPublishReq, Uint8Array]>{
        return super.raw_decode(buf).map(r=>{
            const [obj, _buf] = r;
            const sub_obj = new TopicPublishReq(obj.desc(),obj.body(), obj.signs(), obj.nonce());
            return [sub_obj, _buf] as [TopicPublishReq, Uint8Array];
        });
    }
}