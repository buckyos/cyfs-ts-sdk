...
export class BuckyNumber implements RawEncode {
    ...
    constructor(type: Int, val: bigint | number) {
        this.type = type;
    ...
    raw_encode(buf: Uint8Array, littleEndian?: boolean): BuckyResult<Uint8Array> {
        ...
        switch (this.type) {
            ...
            case "i8": {
                view.setInt8(0, Number(val));
                return Ok(buf.offset(1));
            }
            case "u16": {
                view.setUint16(0, Number(val), littleEndian);
                return Ok(buf.offset(2));
            }
            case "i16": {
                view.setInt16(0, Number(val), littleEndian);
                return Ok(buf.offset(2));
            }
            case "u32": {
                view.setUint32(0, Number(val), littleEndian);
                return Ok(buf.offset(4));
            }
            case "i32": {
                view.setInt32(0, Number(val), littleEndian);
                return Ok(buf.offset(4));
            }
            case "u64": {
            ...
            case "u128": {
                ...
                const left = val / U64MAX;
                const right = val % U64MAX;
...
export class BuckyNumberDecoder implements RawDecode<BuckyNumber>{
    ...
    raw_decode(buf: Uint8Array, littleEndian?: boolean): BuckyResult<[BuckyNumber, Uint8Array]> {
        ...
        switch (this.type) {
            case "u8": {
                val = BigInt(view.getUint8(0));
                buf = buf.offset(1);
                break;
            }
            case "i8": {
                val = BigInt(view.getInt8(0));
                buf = buf.offset(1);
                break;
            }
            case "u16": {
                val = BigInt(view.getUint16(0, littleEndian));
                buf = buf.offset(2);
                break;
            }
            case "i16": {
                val = BigInt(view.getInt16(0, littleEndian));
                buf = buf.offset(2);
                break;
            }
            case "u32": {
                val = BigInt(view.getUint32(0, littleEndian));
                buf = buf.offset(4);
                break;
            }
            case "i32": {
                val = BigInt(view.getInt32(0, littleEndian));
                buf = buf.offset(4);
                break;
            }
            case "u64": {
