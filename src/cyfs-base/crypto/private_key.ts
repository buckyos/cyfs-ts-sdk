...
export abstract class PrivateKey implements RawEncode{
    ...
    static generate_rsa(bits: number): BuckyResult<PrivateKey> {
        ...
        return Ok(new RSAPrivateKey(code, ret.privateKey));
    }
...
export class RSAPrivateKey extends PrivateKey{
    constructor(public code: number, public value: crypto.KeyObject) {
        super(KEY_TYPE_RSA)
    }
    raw_measure(ctx?: any, purpose?: RawEncodePurpose): BuckyResult<number> {
        let buf = this.value.export({type: 'pkcs1', format: "der"});
    ...
    raw_encode(buf: Uint8Array, ctx?: any, purpose?: RawEncodePurpose): BuckyResult<Uint8Array> {
        ...
        let der_key = this.value.export({type: 'pkcs1', format: "der"});
    ...
    public(): PublicKey {
        let pub_key = crypto.createPublicKey(this.value);
        let pub_buf = new Uint8Array(pub_key.export({type: 'pkcs1', format: 'der'}))
        return new RSAPublicKey(this.code, pub_buf);
    }
    sign(data: Uint8Array, sign_source: SignatureSource): Signature {
        ...
        let sign_buf = crypto.sign('RSA-SHA256', data_new, {
            key: this.value,
    ...
    decrypt(input: Uint8Array, out: Uint8Array): BuckyResult<number> {
        let ret = crypto.privateDecrypt(this.value, input);
        out.set(ret);
            ...
            case KEY_TYPE_RSA:
                {
                    ...
                    let pk = crypto.createPrivateKey({
                        key: Buffer.from(buf.slice(0, len.toNumber())),
                        format: 'der',
                        type: 'pkcs1'
                    });
                    buf = buf.offset(len.toNumber());
                    // TODO: Node < 15的版本取不到公私钥的bits长度，这里随便填一个.....
                    let rsa = new RSAPrivateKey(RAW_PUBLIC_KEY_RSA_1024_CODE, pk);
