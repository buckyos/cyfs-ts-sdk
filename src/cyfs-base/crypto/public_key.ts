...
export class Signature implements RawEncode {
    ...
    m_sign_time: bigint; // u64;
    m_sign: SignData;

    constructor(sign_source: SignatureSource, sign_key_index: number, sign_time: bigint, sign: SignData) {
    ...
    raw_measure(): BuckyResult<number> {
        ...
        const r = this.sign_source.match<BuckyResult<number>>({
            ...
            Object: (obj) => {
                ...
                if (r.err) {
                    base_error("raw_measure SignatureSource Object Type failed， err:{}", r.err);
                    return r;
                }
                return r;
            },
    ...
    raw_encode(buf: Uint8Array): BuckyResult<Uint8Array> {
        // sign_source_with_ref_index: u8
        {
            let index = this.sign_source_with_ref_index();
            buf[0] = index;
            buf = buf.offset(1);
        }
        ...
        // signatory: Option<SignatureSource>
        {
            ...
            if (r.err) {
                base_error("encode signature sign_source failed, err:{}", r.err);
                return r;
            }
            buf = r.unwrap();
        }
        ...
        // sign_time: u64
        {
            ...
            if (r.err) {
                base_error("encode signature sign_time failed, err:{}", r.err);
                return r;
            }
            buf = r.unwrap();
        }
    ...
    raw_decode(buf: Uint8Array): BuckyResult<[Signature, Uint8Array]> {
        ...
        switch (sign_source_code) {
            ...
            case SIGNATURE_OBJECT: {
                ...
                if (r.err) {
                    base_error("Signature decode soruce object failed, err:{}", r.err);
                    return r;
                }
                let link: ObjectLink;
                [link, buf] = r.unwrap();
            ...
            case SIGNATURE_KEY: {
                ...
                if (r.err) {
                    base_error("Signature decode soruce key failed, err:{}", r.err);
                    return r;
                }
        ...
        let sign_time;
        {
            ...
            if (r.err) {
                base_error("Signature decode sign_time failed, err:{}", r.err);
                return r;
            }
