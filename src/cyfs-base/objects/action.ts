...
export class Action extends NamedObject<ActionDescContent, ActionBodyContent>{
    static create(build?:(builder: ActionBuilder)=>void):Action{
        ...
        return new Action(self.desc(), self.body(), self.signs(), self.nonce());
    }
...
export class ActionDecoder extends NamedObjectDecoder<ActionDescContent, ActionBodyContent, Action>{
    ...
    raw_decode(buf: Uint8Array): BuckyResult<[Action, Uint8Array]>{
        return super.raw_decode(buf).map(r=>{
            const [obj, _buf] = r;
            const sub_obj = new Action(obj.desc(),obj.body(), obj.signs(), obj.nonce());
            return [sub_obj, _buf] as [Action, Uint8Array];
        });
    }
}