...
export function number_2_obj_type_code(x:number): ObjectTypeCode{
    if (typeof ObjectTypeCode[x] === 'undefined') {
        // base_error('Invalid ObjectTypeCode number');
...
export function number_2_obj_type_code_name(x:number): string{
    ...
    switch(x){
        ...
        case ObjectTypeCode.Relation: return "ObjectTypeCode.Relation";
        case ObjectTypeCode.Contract: return "ObjectTypeCode.Contract";
        default: return "ObjectTypeCode.Custom";
    }
}

export function obj_type_code_raw_check(buf:Uint8Array):BuckyResult<ObjectTypeCode>{
    const flag = buf[0];
    // 11000000
    if((flag>>6)!==1){
        return Ok(ObjectTypeCode.Custom);
    }else{
        // obj_bits[. .]type_code[. . . .] country[. .]
        // 63 => 111111
        const obj_type_code = ((flag&63)>>2);
        return Ok(number_2_obj_type_code(obj_type_code));
    }
}
