...
export class ProofOfService extends NamedObject<ProofOfServiceDescContent, ProofOfServiceBodyContent>{
    ...
    static create(owner: Option<ObjectId>, proof_type: ProofTypeCode, data_0: ProofData, data_1: ProofData): ProofOfService{
        ...
        // 构造，这是一个有主对象
        const self:NamedObject<ProofOfServiceDescContent, ProofOfServiceBodyContent> =
            builder
            .option_owner(owner)
            .build();

        // 这是一个绕过typescript类型的trick，通过重新调用对象构造函数（继承自父对象）, 使得返回的对象类型是具体化后的ProofOfService
        return new ProofOfService(self.desc(), self.body(), self.signs(), self.nonce());
    }
...
export class ProofOfServiceDecoder extends NamedObjectDecoder<ProofOfServiceDescContent, ProofOfServiceBodyContent, ProofOfService>{
    ...
    raw_decode(buf: Uint8Array): BuckyResult<[ProofOfService, Uint8Array]>{
        return super.raw_decode(buf).map(r=>{
            const [obj, _buf] = r;
            const sub_obj = new ProofOfService(obj.desc(),obj.body(), obj.signs(), obj.nonce());
            return [sub_obj, _buf] as [ProofOfService, Uint8Array];
        });
    }
}
