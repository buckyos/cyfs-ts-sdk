...
export class AppGroup extends NamedObject<AppGroupDescContent, AppGroupBodyContent>{
    static create(build?:(builder:AppGroupBuilder)=>void):AppGroup{
        ...
        return new AppGroup(self.desc(), self.body(), self.signs(), self.nonce());
    }
...
export class AppGroupDecoder extends NamedObjectDecoder<AppGroupDescContent, AppGroupBodyContent, AppGroup>{
    ...
    raw_decode(buf: Uint8Array): BuckyResult<[AppGroup, Uint8Array]>{
        return super.raw_decode(buf).map(r=>{
            const [obj, _buf] = r;
            const sub_obj = new AppGroup(obj.desc(),obj.body(), obj.signs(), obj.nonce());
            return [sub_obj, _buf] as [AppGroup, Uint8Array];
        });
    }
}