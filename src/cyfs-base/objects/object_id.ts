...
import { RawEncode, RawDecode, DecodeBuilder, Compareable } from "../base/raw_encode";
...
import { PublicKeyValue, PublicKey, MNPublicKey, PublicKeyValueDecoder, Signature } from "../crypto/public_key";
import { bucky_time_now } from "../base/time";
import bs58 from 'bs58';
//const bs58 = require('../base/bs58');

export const OBJECT_ID_LEN: number = 32;

export const OBJECT_ID_STANDARD: number = 1;
export const OBJECT_ID_CORE: number = 2;
export const OBJECT_ID_DEC_APP: number = 3;

export const OBJECT_ID_FLAG_AREA: number = 1 << 3;
export const OBJECT_ID_FLAG_PK: number = 1 << 2;
export const OBJECT_ID_FLAG_MN_PK: number = 1 << 1;
export const OBJECT_ID_FLAG_OWNER: number = 1;
...
export class ObjectId implements RawEncode, Compareable<ObjectId> {
    ...
    obj_type_code(): Option<ObjectTypeCode> {
        const ret = obj_type_code_raw_check(this.as_slice());
        if (ret.err) {
            return None;
        } else {
