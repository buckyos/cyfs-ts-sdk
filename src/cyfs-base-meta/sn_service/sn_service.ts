...
export class SNServiceDescTypeInfo extends DescTypeInfo{
    ...
    // 每个对象需要一个应用App唯一的编号
    obj_type() : number{
        return CoreObjectType.SNService;
    }
...
export class SNServiceId extends NamedObjectId<SNServiceDescContent, SNServiceBodyContent>{
    constructor(id: ObjectId){
        super(CoreObjectType.SNService, id);
    }

    static default(): SNServiceId{
        return named_id_gen_default(CoreObjectType.SNService);
    }

    static from_base_58(s: string): BuckyResult<SNServiceId> {
        return named_id_from_base_58(CoreObjectType.SNService, s);
    }

    static try_from_object_id(id: ObjectId): BuckyResult<SNServiceId>{
        return named_id_try_from_object_id(CoreObjectType.SNService, id);
    }
...
export class SNServiceIdDecoder extends NamedObjectIdDecoder<SNServiceDescContent, SNServiceBodyContent>{
    constructor(){
        super(CoreObjectType.SNService);
    }
...
export class SNService extends NamedObject<SNServiceDescContent, SNServiceBodyContent>{
    // 提供一个静态的创建方法
    static create(owner: ObjectId, service_type: number, price: bigint): SNService{
        // 创建DescContent部分
        ...
        // 构造，这是一个有主对象
        const self:NamedObject<SNServiceDescContent, SNServiceBodyContent> =
            builder
            .owner(owner)
            .build();

        // 这是一个绕过typescript类型的trick，通过重新调用对象构造函数（继承自父对象）, 使得返回的对象类型是具体化后的SNService
        return new SNService(self.desc(), self.body(), self.signs(), self.nonce());
    }
...
export class SNServiceDecoder extends NamedObjectDecoder<SNServiceDescContent, SNServiceBodyContent, SNService>{
    ...
    raw_decode(buf: Uint8Array): BuckyResult<[SNService, Uint8Array]>{
        return super.raw_decode(buf).map(r=>{
            const [obj, _buf] = r;
            const sub_obj = new SNService(obj.desc(),obj.body(), obj.signs(), obj.nonce());
            return [sub_obj, _buf] as [SNService, Uint8Array];
        });
    }
}
