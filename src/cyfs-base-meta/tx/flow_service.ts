...
export class FlowServiceDescTypeInfo extends DescTypeInfo{
    ...
    // 每个对象需要一个应用App唯一的编号
    obj_type() : number{
        return CoreObjectType.FlowService;
    }
...
export class FlowServiceId extends NamedObjectId<FlowServiceDescContent, FlowServiceBodyContent>{
    constructor(id: ObjectId){
        super(CoreObjectType.FlowService, id);
    }

    static default(): FlowServiceId{
        return named_id_gen_default(CoreObjectType.FlowService);
    }

    static from_base_58(s: string): BuckyResult<FlowServiceId> {
        return named_id_from_base_58(CoreObjectType.FlowService, s);
    }

    static try_from_object_id(id: ObjectId): BuckyResult<FlowServiceId>{
        return named_id_try_from_object_id(CoreObjectType.FlowService, id);
    }
...
export class FlowServiceIdDecoder extends NamedObjectIdDecoder<FlowServiceDescContent, FlowServiceBodyContent>{
    constructor(){
        super(CoreObjectType.FlowService);
    }
...
export class FlowService extends NamedObject<FlowServiceDescContent, FlowServiceBodyContent>{
    // 提供一个静态的创建方法
    static create(owner: ObjectId, price: bigint): FlowService{
        // 创建DescContent部分
        ...
        // 构造，这是一个有主对象
        const self:NamedObject<FlowServiceDescContent, FlowServiceBodyContent> =
            builder
            .owner(owner)
            .build();

        // 这是一个绕过typescript类型的trick，通过重新调用对象构造函数（继承自父对象）, 使得返回的对象类型是具体化后的FlowService
        return new FlowService(self.desc(), self.body(), self.signs(), self.nonce());
    }
...
export class FlowServiceDecoder extends NamedObjectDecoder<FlowServiceDescContent, FlowServiceBodyContent, FlowService>{
    ...
    raw_decode(buf: Uint8Array): BuckyResult<[FlowService, Uint8Array]>{
        return super.raw_decode(buf).map(r=>{
            const [obj, _buf] = r;
            const sub_obj = new FlowService(obj.desc(),obj.body(), obj.signs(), obj.nonce());
            return [sub_obj, _buf] as [FlowService, Uint8Array];
        });
    }
}
