...
export class MinerGroupDescTypeInfo extends DescTypeInfo{
    ...
    // 每个对象需要一个应用App唯一的编号
    obj_type() : number{
        return CoreObjectType.MinerGroup;
    }
...
export class MinerGroupId extends NamedObjectId<MinerGroupDescContent, MinerGroupBodyContent>{
    constructor(id: ObjectId){
        super(CoreObjectType.MinerGroup, id);
    }

    static default(): MinerGroupId{
        return named_id_gen_default(CoreObjectType.MinerGroup);
    }

    static from_base_58(s: string): BuckyResult<MinerGroupId> {
        return named_id_from_base_58(CoreObjectType.MinerGroup, s);
    }

    static try_from_object_id(id: ObjectId): BuckyResult<MinerGroupId>{
        return named_id_try_from_object_id(CoreObjectType.MinerGroup, id);
    }
...
export class MinerGroupIdDecoder extends NamedObjectIdDecoder<MinerGroupDescContent, MinerGroupBodyContent>{
    constructor(){
        super(CoreObjectType.MinerGroup);
    }
...
export class MinerGroup extends NamedObject<MinerGroupDescContent, MinerGroupBodyContent>{
    ...
    static create(members: DeviceDesc[]): MinerGroup{
        ...
        // 构造，这是一个有主对象
        const self:NamedObject<MinerGroupDescContent, MinerGroupBodyContent> =
            builder
            .build();

        // 这是一个绕过typescript类型的trick，通过重新调用对象构造函数（继承自父对象）, 使得返回的对象类型是具体化后的MinerGroup
        return new MinerGroup(self.desc(), self.body(), self.signs(), self.nonce());
    }
...
export class MinerGroupDecoder extends NamedObjectDecoder<MinerGroupDescContent, MinerGroupBodyContent, MinerGroup>{
    ...
    raw_decode(buf: Uint8Array): BuckyResult<[MinerGroup, Uint8Array]>{
        return super.raw_decode(buf).map(r=>{
            const [obj, _buf] = r;
            const sub_obj = new MinerGroup(obj.desc(),obj.body(), obj.signs(), obj.nonce());
            return [sub_obj, _buf] as [MinerGroup, Uint8Array];
        });
    }
}
