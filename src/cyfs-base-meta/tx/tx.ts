...
// 自定义BodyContent
export class TxBodyContent extends BodyContent{
    constructor(
...
export class Tx extends NamedObject<MetaTxDescContentType, TxBodyContent>{
    ...
    // 提供一个静态的创建方法
    static create(nonce: bigint, caller: TxCaller, gas_coin_id: number, gas_price: number, max_fee: number, condition: Option<TxCondition>, body: MetaTxBody, data: Uint8Array): Tx{
        // 创建DescContent部分
        ...
        return new Tx(self.desc(), self.body(), self.signs(), self.nonce());
    }
...
export class TxDecoder extends NamedObjectDecoder<MetaTxDescContentType, TxBodyContent, Tx>{
    ...
    raw_decode(buf: Uint8Array): BuckyResult<[Tx, Uint8Array]>{
        return super.raw_decode(buf).map(r=>{
            const [obj, _buf] = r;
            const sub_obj = new Tx(obj.desc(),obj.body(), obj.signs(), obj.nonce());
            return [sub_obj, _buf] as [Tx, Uint8Array];
        });
    }
}
